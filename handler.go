...
type GzipResponseWriter struct {
	...
	accept acceptsType
	index  int // Index for gzipWriterPools/brotliWritePools.
	gw     *gzip.Writer
	bw     *brotli.Writer
...
func (w *GzipResponseWriter) Write(b []byte) (int, error) {
	...
	if ce == "" && (cl == 0 || cl >= w.minSize) && (ct == "" || handleContentType2(w.contentTypes, w.contentTypes, ct) != handleNone) {
...
func GzipHandlerWithOpts(opts ...option) (func(http.Handler) http.Handler, error) {
	...
	return func(h http.Handler) http.Handler {
		index := gzipPoolIndex(c.level)

		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Add(vary, acceptEncoding)
			if ac := acceptsCompression(r); ac.gzip() {
